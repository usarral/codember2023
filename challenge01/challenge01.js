...
const arrayWords = rawInput.split(' '); // Get the words as array elements
let contador = arrayWords.reduce((contador, palabra) => {
  palabra = palabra.toLowerCase(); //convert to lowercase for avoid duplicated words

  if (palabra in contador) {
    //count the words
    contador[palabra]++; //if exist increment the counter by one
  } else {
    contador[palabra] = 1; //if not, create a counter for the new element
  }
  return contador; //return the object
}, {});

let answer = ''; //Create the answer script
for (const word in contador) {
  //for each word add it to the answer string using the required format
  answer += word + contador[word];
}
console.log(answer); //show the output of the answer on finish
